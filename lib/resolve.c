...
static int write_extra_records(const rr_array_t *arr, uint16_t reorder, knot_pkt_t *answer)
{
	for (size_t i = 0; i < arr->len; ++i) {
		int err = knot_pkt_put(answer, 0, arr->at[i], KNOT_PF_ROTATE*reorder);
		if (err != KNOT_EOK) {
				      ...
				      knot_pkt_t *answer, bool *all_secure, bool *all_cname)
{
	...
	for (size_t i = 0; i < arr->len; ++i) {
		...
		int err = knot_pkt_put(answer, 0, rr, KNOT_PF_ROTATE*reorder);
		if (err != KNOT_EOK) {
			if (err == KNOT_ESPACE) {
				err = kr_ok();
			}
...
static int answer_finalize(struct kr_request *request, int state)
{
	...
	if (request->answ_selected.len > 0) {
		...
		if (write_extra_ranked_records(&request->answ_selected, last->reorder,
	...
	if (write_extra_ranked_records(&request->auth_selected, last->reorder,
	    answer, &secure, NULL)) {
		return answer_fail(request);
	}
	/* Write additional records. */
	knot_pkt_begin(answer, KNOT_ADDITIONAL);
	if (write_extra_records(&request->additional, last->reorder, answer)) {
		return answer_fail(request);
	}
